...
pos_model_path = '../../../save_model/robotic_hand_real_s4_a6/pos'  # use to import weights and normalization arrays
load_model_path = '../../../save_model/robotic_hand_real_s4_a6/load'
...
with open('../../../data/robotic_hand_real/t42_cyl45_right_test_paths.obj', 'rb') as pickle_file:
    pro, aro = pickle.load(pickle_file, encoding='latin1')

gt = pro[0]
acts = aro[0]

validation_data = np.asarray(gt)
...
if __name__ == "__main__":
    with tf.Session(config=config) as sess:
        ...
        for i in range(len(validation_data)-1):
            ...
            state = np.append(np.append(next_pos, next_load), validation_data[i + 1][4:10])
            nor_state = z_score_normalize(np.asarray([state]), x_nor_arr[0], x_nor_arr[1])
